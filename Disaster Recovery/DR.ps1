...
param (
    ...
    $Recovery_VM_Name,
    [Parameter()][array]
    $Recovery_Storage_Name,
    [Parameter()][array]
    $Recovery_Storage_SKU_Name,
    [Parameter()][array]
    $Recovery_Storage_Kind,
    [Parameter()][array]
    $Recovery_Storage_Access_Tier,
    [Parameter()][array]
    $Recovery_Storage_Https_Traffic,
    [Parameter()][array]
    $Recovery_Storage_Enable_HNS,
    [Parameter()][array]
    $Recovery_Storage_Blob_Access,
    [Parameter()][array]
    $Recovery_Storage_TLS_Version,
    [Parameter()][hashtable]
    $Recovery_Storage_Tags
)
...
$Primary_Vnet_RG_Name     = $Primary_RG_Name
$Primary_Vnet_RG_Location = $Primary_RG_Location
$Primary_Vnet_Tags = @{
    "Env"  = "Testing" ;
    "Name" = "RG"
}
...
$IP_RG_Name           = $Primary_RG_Name * $Public_IP_Name.Count
$IP_RG_Location       = $Primary_RG_Location * $Public_IP_Name.Count
$Public_IP_Tags = @{
    "Env"  = "Testing" ;
    "Name" = "Public-IP"
}
...
$NSG_Name        = @('NSG-1')
$NSG_RG_Name     = $Primary_RG_Name
$NSG_RG_Location = $Primary_RG_Location
$NSG_Tags = @{
    "Env"  = "Testing" ;
    "Name" = "NSG"
}
...
$Primary_Subnet_RG_Name     = $Primary_RG_Name
$Primary_Vnet_Subnet_Name   = $Primary_Vnet_Name

$NIC_Name                = @('NIC-1', 'NIC-2')
$NIC_IP_Cfg_Name         = @('NIC-IP-Cfg-1', 'NIC-IP-Cfg-2')
$NIC_RG_Name             = $Primary_RG_Name * $NIC_Name.Count
$NIC_RG_Location         = $Primary_RG_Location * $NIC_Name.Count
$Primary_Vnet_NIC_Name   = $Primary_Vnet_Name * $NIC_Name.Count
$Primary_Subnet_NIC_Name = $Primary_Subnet_Name * $NIC_Name.Count
$NIC_Tags = @{
    "Env"  = "Testing" ;
    "Name" = "NIC"
}
...
$VM_RG_Name                         = $Primary_RG_Name * $Windows_VM_Name.Count
$VM_RG_Location                     = $Primary_RG_Location * $Windows_VM_Name.Count
...
$Storage_RG_Name               = $Primary_RG_Name
$Storage_RG_Location           = $Primary_RG_Location
$Primary_Storage_Tags = @{
    "Env"  = "Testing" ;
...
$Recovery_Vnet_RG_Name     = $Recovery_RG_Name
$Recovery_Vnet_RG_Location = $Recovery_RG_Location
$Recovery_Vnet_Tags = @{
    "Env"  = "Testing" ;
    "Name" = "RG"
}
...
$Recovery_Subnet_RG_Name     = $Recovery_RG_Name

$RSV_Name        = @('Vault-1')
$RSV_RG_Name     = $Recovery_RG_Name
$RSV_RG_Location = $Recovery_RG_Location
$RSV_Tags = @{
    "Env"  = "Testing" ;
...
$Replicated_Item_Primary_RG_Name           = $Primary_RG_Name * $Replicated_Item_Name.Count
$Replicated_Item_Recovery_RG_Name          = $Recovery_RG_Name * $Replicated_Item_Name.Count
$Replicated_Item_Recovery_RG_Location      = $Recovery_RG_Location * $Replicated_Item_Name.Count
$Replicated_Storage_Name                   = $Primary_Storage_Name * $Replicated_Item_Name.Count
$Replicated_Recovery_Vnet_Name             = $Recovery_Vnet_Name * $Replicated_Item_Name.Count
$Replicated_Primary_ASR_Name               = $Primary_ASR_Name * $Replicated_Item_Name.Count
$Replicated_Primary_ASR_Container_Name     = $Primary_ASR_Container_Name * $Replicated_Item_Name.Count
$Replicated_Primary_Container_Mapping_Name = $Primary_Container_Mapping_Name * $Replicated_Item_Name.Count
$Replicated_Recovery_Subnet_Name           = $Recovery_Subnet_Name * $Replicated_Item_Name.Count

$Recovery_Storage_Name          = @('recoverycachestorage7321')
$Recovery_Storage_SKU_Name      = @('Standard_LRS')
$Recovery_Storage_Kind          = @('StorageV2')
$Recovery_Storage_Access_Tier   = @('Hot')
$Recovery_Storage_Https_Traffic = @($True)
$Recovery_Storage_Enable_HNS    = @($False)
$Recovery_Storage_Blob_Access   = @($True)
$Recovery_Storage_TLS_Version   = @('TLS1_2')
$Recovery_Storage_RG_Name       = $Recovery_RG_Name
$Recovery_Storage_RG_Location   = $Recovery_RG_Location
$Recovery_Storage_Tags = @{
    "Env"  = "Testing" ;
    "Name" = "Storage Account"
}

$Replicated_Recovery_Storage_Name       = $Recovery_Storage_Name * $Replicated_Item_Name.Count
$Replicated_Recovery_ASR_Name           = $Recovery_ASR_Name * $Replicated_Item_Name.Count
$Replicated_Recovery_ASR_Container_Name = $Recovery_ASR_Container_Name * $Replicated_Item_Name.Count
$Recovery_Container_Mapping_Name        = $Recovery_Container_Mapping_Name * $Replicated_Item_Name.Count
$Replicated_Item_Primary_RG_Location    = $Primary_RG_Location * $Replicated_Item_Name.Count 
...
for ($i = 0; $i -lt $RSV_Name.Count; $i++) {
    ...
    $ReplicationProtectedItem = Get-AzRecoveryServicesAsrReplicationProtectedItem -ProtectionContainer $PrimaryProtectionContainer | Select-Object ProtectionState
...
for ($i = 0; $i -lt $Replicated_Item_Name.Count; $i++) {
    ...
    $null = "{0} {1}" -f $RecoveryPoint[0].RecoveryPointType, $RecoveryPoint[-1].RecoveryPointTime
...
#Creation of Storage Account in Recovery Resource Group
for ($i = 0; $i -lt $Recovery_Storage_Name.Count; $i++) {
    Write-Output "`t`tCreating Cache Storage Account in Recovery Resource Group" 
    $null = New-AzStorageAccount `
        -ResourceGroupName           $Recovery_Storage_RG_Name[$i] `
        -Name                        $Recovery_Storage_Name[$i] `
        -SkuName                     $Recovery_Storage_SKU_Name[$i] `
        -Location                    $Recovery_Storage_RG_Location[$i] `
        -Kind                        $Recovery_Storage_Kind[$i] `
        -AccessTier                  $Recovery_Storage_Access_Tier[$i] `
        -EnableHttpsTrafficOnly      $Recovery_Storage_Https_Traffic[$i] `
        -EnableHierarchicalNamespace $Recovery_Storage_Enable_HNS[$i] `
        -AllowBlobPublicAccess       $Recovery_Storage_Blob_Access[$i] `
        -MinimumTlsVersion           $Recovery_Storage_TLS_Version[$i] `
        -Tag                         $Recovery_Storage_Tags

    Write-Output "`t`tCache Storage Account in Recovery Resource Group is Created`n"
}
...
for ($i = 0; $i -lt $Replicated_Item_Name.Count; $i++) {
    ...
    #Get Recovery Storage Account
    Write-Output "`t`tGetting Recovery Storage Account`n"
    $RecoveryStorage = Get-AzStorageAccount -Name $Replicated_Recovery_Storage_Name[$i] -ResourceGroupName $Replicated_Item_Recovery_RG_Name[$i]

    #Get Recovery ASR
    Write-Output "`t`tGetting Recovery ASR Fabric`n"
    $RecoveryFabric = Get-AzRecoveryServicesAsrFabric -Name $Replicated_Recovery_ASR_Name[$i]
    
    #Get Recovery Protection Container
    Write-Output "`t`tGetting Recovery Protection Container`n"
    $RecoveryProtectionContainer = Get-AzRecoveryServicesAsrProtectionContainer -Fabric $RecoveryFabric -Name $Replicated_Recovery_ASR_Container_Name[$i]

    #Get Recovery to Primary Mapping for reverse replication
    Write-Output "`t`tGetting Recovery to Primary Mapping for Reverse Replication of replicated item`n"
    $RectoPriPCMapping = Get-AzRecoveryServicesAsrProtectionContainerMapping -Name $Recovery_Container_Mapping_Name[$i] -ProtectionContainer $RecoveryProtectionContainer

    #Get Primary Resource Group
    Write-Output "`t`tGetting Primary Resource Group`n"
    $PrimaryRG = Get-AzResourceGroup -Name $Replicated_Item_Primary_RG_Name[$i] -Location $Replicated_Item_Primary_RG_Location[$i]

    #Starting Reverse Replication for the replication protected item 
    Write-Output "`t`tStarting Reverse Replication for the replicaton protected item`n"
    $ASRJob = Update-AzRecoveryServicesAsrProtectionDirection -ReplicationProtectedItem $ReplicationProtectedItem -AzureToAzure -ProtectionContainerMapping $RectoPriPCMapping -LogStorageAccountId $RecoveryStorage.Id -RecoveryResourceGroupID $PrimaryRG.ResourceId

    while (($ASRJob.State -eq 'InProgress') -or ($ASRJob.State -eq 'NotStarted')) {
        Start-Sleep 10;
        $ASRJob = Get-AzRecoveryServicesAsrJob -Job $ASRJob
    }
    Write-Output "`t`tReverse Replication for the replication protected item is completed`n"
}