...
{
    Write-Output "Stopping $($Pool) in $($SynpaseWSName) Synapse Workspace"
    Suspend-AzSynapseSqlPool -WorkspaceName $SynpaseWSName -Name $Pool
    Write-Output "$($Pool) is Stopped in $($SynpaseWSName) Synapse Workspace"
}
